...
import { calculateSurfaceScores, driveNames } from "@/lib/core-utils/fit-core";

export default function SurfacePersonaReport() {
  const supabase = createBrowserSupabaseClient();
  const [data, setData] = useState<any>(null);
  
  // Theme Colors
  const themeBlue = "#7c94be"; // Muted Blue for Surface Page Theme
  const sage = "#93a97c";
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (testData) {
        ...
        const scores = calculateSurfaceScores(testData);
                  ...
                  dominantDrives.map((drive: any) => (
                    <div key={drive.name} className="border-l-4 pl-6" style={{ borderColor: themeBlue }}>
                      <p className="text-[13px] text-slate-700 leading-relaxed font-medium">
                        ...
                        In your own surface perception, you prioritize {drive.desc}
                      </p>
                    </div>
                  ))
                ) : (
