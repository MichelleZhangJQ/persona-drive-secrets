...
function titleCaseBasis(b: string) {
  return b === "innate" ? "innate" : b === "surface" ? "surface" : b;
}
...
function formatComponentLabel(params: {
  ...
  const { c, rankInnate, rankSurface } = params;
  const r =
    c.basis === "innate" ? rankInnate?.[c.selfDrive] : c.basis === "surface" ? rankSurface?.[c.selfDrive] : null;

  const rankStr = r ? ` (rank #${r.rank}, ${fmt(r.score, 2)})` : "";
  return `${titleCaseBasis(c.basis)} ${c.selfDrive}${rankStr}`;
}
...
function formatCapByLine(params: {
  ...
  const { capInfo, rankInnate, rankSurface } = params;
  ...
  // 1) Prefer explicit bindingComponents (best)
  if (Array.isArray(binding) && binding.length > 0) {
    const labels = binding.map((c) => formatComponentLabel({ c, rankInnate, rankSurface }));
    const uniq = Array.from(new Set(labels));

    if (uniq.length === 1) return `Capped by (6 - your ${uniq[0]} drive)`;
    if (uniq.length === 2) return `Capped by  (6- your ${uniq[0]} and 6- your ${uniq[1]} drive)`;
    return `Capped by your (6- your ${uniq.slice(0, -1).join(", ")}), and (6- your ${uniq[uniq.length - 1]} drive)` ;
  }

  // 2) Otherwise infer binding = those with cap == final cap
  if (Array.isArray(capComps) && capComps.length > 0) {
    ...
    if (inferredBinding.length > 0) {
      const labels = inferredBinding.map((c) => formatComponentLabel({ c, rankInnate, rankSurface }));
      const uniq = Array.from(new Set(labels));

      if (uniq.length === 1) return `Capped by (6 - your ${uniq[0]} drive)`;
      if (uniq.length === 2) return `Capped by your (6- your ${uniq[0]}) and (6- your ${uniq[1]} drive)`;
      return `Capped by  (6- your ${uniq.slice(0, -1).join(", ")}), and (6- your ${uniq[uniq.length - 1]} drive)`
    }
  }

  // 3) Final fallback: primary (basis/selfDrive)
  ...
  return `Capped by your ${titleCaseBasis(basis)} ${selfDrive}${r ? ` (rank #${r.rank}, ${fmt(r.score, 2)})` : ""}`;
}
...
function formatCapParagraph(params: {
  ...
  const { partnerDrive, capInfo, rankInnate, rankSurface } = params;
  ...
  // Prefer explicit bindingComponents; else infer from final cap
  ...
  // If compute sets legacy "primary" (capInfo.basis/selfDrive), prefer that one when ties exist
  const chosen =
    inferredBinding.find((c) => c.basis === capInfo.basis && c.selfDrive === capInfo.selfDrive) ?? inferredBinding[0];

  const label = formatComponentLabel({ c: chosen, rankInnate, rankSurface });
  const because = (chosen.reason || "").trim();

  if (because) {
    return `Your partner’s ${partnerDrive} is capped by your ${label} drive, because ${because}.`;
  }
  return `Your partner’s ${partnerDrive} is capped by your ${label} drive.`;
}
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const [profileRes, innateRes, surfaceRes] = await Promise.all([
  ...
  if (loading)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 text-slate-400">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest italic">Computing Ideal Partner Profile...</p>
      </div>
    );
      ...
      <div className="min-h-screen bg-[#FDFDFD]">
        ...
        <main className="max-w-4xl mx-auto px-6 py-16">
          <div className="rounded-3xl border border-slate-200 bg-white p-8 text-slate-600 font-medium">
            Missing persona data (innate or surface). Please complete the tests first.
          </div>
        </main>
      </div>
    );
        ...
        <section className="mb-12">
          <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="text-center md:text-left">
              <h1 className="text-5xl font-black uppercase italic text-slate-900 leading-none">
                Ideal Partner <span className="text-indigo-600">Profile</span>
              </h1>
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 mt-4">
                Partner drive priorities inferred from your surface + innate needs
              </p>
            </div>
          ...
          <div className="bg-indigo-50/50 border border-indigo-100 rounded-3xl p-8 md:p-10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="text-[11px] font-black uppercase tracking-widest text-indigo-900 italic">How to read</div>
            </div>
            <p className="text-sm text-indigo-900/80 leading-relaxed font-medium">
                On this page, you’ll first see your <strong>Ideal Partner Drive Profile</strong> as a bar chart.
            </p>

            <p className="text-sm text-indigo-900/80 leading-relaxed font-medium">
                Then—starting with the <strong>most important partner drives</strong>—each section explains:
            </p>

            <ul className="mt-2 space-y-1 pl-5 list-disc text-sm text-indigo-900/80 leading-relaxed font-medium">
               <li>
                what that <strong>partner drive</strong> tends to provide for you,
              </li>
               <li>
                which of <strong>your drives</strong> it supports, and
               </li>
               <li>
                where those drives rank in your <strong>Innate</strong> and <strong>Surface</strong> profiles.
               </li>
            </ul>

            <p className="mt-4 text-sm text-indigo-900/80 leading-relaxed font-medium">
                If a partner drive is marked <strong>Capped</strong>, it means <strong>your own profile sets an upper limit</strong> on
                how intense that partner drive should be. In other words, “lower” can be intentional—because too much of that partner drive
                would backfire for you.
            </p>

            <p className="mt-4 text-sm text-indigo-900/80 leading-relaxed font-medium">
                This report doesn’t claim that partners with different profiles can’t lead to a great relationship. It simply
                highlights the traits that are <em>most consistently meaningful for you</em>, given your current patterns. The
                rankings also aren’t absolute—real life is complex, and different situations can bring out different needs.
            </p>

            <p className="mt-4 text-sm text-indigo-900/80 leading-relaxed font-medium">
                In this model, we treat <strong>Care</strong>, <strong>Exploration</strong>, <strong>Affiliation</strong>, and{" "}
                <strong>Value</strong> as <strong>innate drives</strong>, and <strong>Achievement</strong>, <strong>Dominance</strong>,
                and <strong>Pleasure</strong> as <strong>surface drives</strong>. Your ideal partner drive levels are then estimated
                from your <strong>innate</strong> and <strong>surface</strong> persona profiles, depending on which of your needs each
                partner drive is supporting.
            </p>
        ...
        <section className="mb-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex items-start justify-between gap-4 mb-6">
                <div>
                  <div className="text-2xl font-black uppercase italic text-slate-900 leading-none">
                    Ideal Partner <span className="text-indigo-600">Surface</span> Profile
                  </div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-2">
                    Ranked in the same order as the innate chart. Scale is 0–5.
                  </div>
                </div>
              </div>
            ...
            <div className="mt-6 text-[11px] text-slate-600 font-medium leading-relaxed">
              These are <strong>estimated traits</strong> for your partner’s <strong>surface expressed</strong> drives. We highlight{" "}
              <strong>Dominance</strong>, <strong>Achievement</strong>, <strong>Pleasure</strong>, and <strong>Affiliation</strong>{" "}
              as the surface-prioritized set here. Your partner’s expressed strengths in these areas shape how you interact with each other.
              The remaining drives are shown in grey.
            </div>
            </div>
            ...
            <div className="rounded-[2.5rem] border-2 border-slate-200 bg-white p-8 md:p-10 shadow-sm">
              <div className="flex items-start justify-between gap-4 mb-6">
                <div>
                  <div className="text-2xl font-black uppercase italic text-slate-900 leading-none">
                    Ideal Partner <span className="text-indigo-600">Innate</span> Profile
                  </div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-2">
                    Ranked in the same order as the surface chart. Scale is 0–5.
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 text-[11px] text-slate-600 font-medium leading-relaxed">
                These are <strong>estimated traits</strong> for your partner’s <strong>innate</strong> style: we highlight
                <strong> Exploration</strong>, <strong>Value</strong>, and <strong>Care</strong> as innately important in
                relationships. The other drives are shown in grey using the same estimated values—because their expression can
                be more situational or environment-dependent.
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="rounded-[2.5rem] border-2 border-slate-200 bg-white p-8 md:p-10 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="text-[20px] font-black uppercase tracking-widest text-slate-700">
                Partner drive explanations
              </div>
            </div>
            <p className="text-[18px] text-slate-600 font-medium leading-relaxed">
              In the following, partner drives are listed by the order of their importance to you.
            </p>
          </div>
              ...
              <div className="relative z-10">
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-3">
                      <div className="text-3xl font-black uppercase italic text-slate-900">{drive}</div>
                ...
                {Array.isArray(x.notes) && x.notes.length > 0 ? (
                  <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4">
                    ...
                    <ul className="mt-2 space-y-1 text-[11px] text-slate-700 font-medium list-disc pl-5">
                      {x.notes.map((t: string, nIdx: number) => (
                        <li key={`note-${drive}-${nIdx}`}>{t}</li>
                      ))}
                    </ul>
                  </div>
                ) : null}
                ...
                <div className="mt-8">
                  <div className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-4">
                    Supported drives and your rankings
                  </div>
                    ...
                    <table className="w-full text-left">
                      <thead className="bg-slate-50">
                        <tr className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                          <th className="px-5 py-3">Supported Drive</th>
                          <th className="px-5 py-3">Innate Persona Ranking</th>
                          <th className="px-5 py-3">Surface Persona Ranking</th>
                          <th className="px-5 py-3">Support Type</th>
                        </tr>
                      </thead>
                        ...
                        {rows.length === 0 ? (
                          <tr>
                            <td className="px-5 py-4 text-[11px] text-slate-500 font-medium" colSpan={4}>
                              No supported drives listed for this partner drive.
                            </td>
                          </tr>
                        ) : (
                          rows.map((d) => {
                            ...
                            const supportType =
                              isInnate && isSurface ? "Innate + Surface" : isInnate ? "Innate" : isSurface ? "Surface" : "—";
          ...
          <div className="bg-slate-100 border border-slate-200 rounded-[3rem] p-10 md:p-14 text-slate-900 shadow-xl relative overflow-hidden">
            <div className="relative z-10 space-y-6">
              ...
              <p className="text-sm font-medium leading-relaxed text-slate-600">
                Use these as relationship “anchors”: they describe what tends to stabilize you. If something is marked
                “capped,” it’s not a lack—it’s your system saying “more of this would backfire.”
              </p>
