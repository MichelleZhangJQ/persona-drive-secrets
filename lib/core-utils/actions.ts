...
function getTargetTable(testType: TestType): string {
  switch (testType) {
    case 'imposed-persona': return 'imposed-persona';
    case 'surface-persona': return 'surface-persona';
...
export async function submitTestData(test_type: TestType, data: TestData) {
  ...
  if (userError || !user) return { success: false, message: "no_user" };
  ...
  const dataToSave = { user_id: user.id, updated_at: nowIso, ...data };
  ...
  revalidatePath('/reports/imposed-persona');
  revalidatePath('/');
